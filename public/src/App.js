...
function App() {
  ...
  const calculate = () => {
    ...
    const exchangeTx = (0.00345 / 100) * turnover;
    const gst = 0.18 * (brokerage + exchangeTx);
    const sebi = (0.000001 * turnover);
    const stamp = (0.003 / 100) * buy * qty;

    const charges = brokerage + stt + exchangeTx + gst + sebi + stamp;
    const profit = (sell - buy) * qty - charges;
    ...
    <div style={{ padding: "20px", fontFamily: "Arial" }}>
      ...
      {result && (
        <div style={{ marginTop: "20px" }}>
          ...
          <p><b>Net Profit/Loss:</b> ₹{result.profit}</p>
        </div>
      )}
    </div>
  );
